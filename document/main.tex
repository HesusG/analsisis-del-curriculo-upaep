...
\setlength{\tabcolsep}{4pt}  % Reduce espacio horizontal en celdas
\renewcommand{\arraystretch}{0.9}  % Reduce espacio vertical entre filas
...
Se propone incorporar diálogos estructurados con asistentes de IA que, mediante preguntas socráticas, guíen al estudiante a reflexionar sobre las implicaciones éticas de sus decisiones de segmentación. El estudiante exporta su conversación, escribe una reflexión breve, y la comparte en un foro de Canvas respondiendo a dos compañeros. Esta estructura integra los cuatro procesos de conocimiento de \citet{Cope2009}: experimentar, conceptualizar, analizar y aplicar.

Para los análisis con datos reales, se recomienda utilizar plataformas de datos abiertos como Kaggle (datasets de segmentación y comportamiento de consumidores), INEGI y datos.gob.mx para contexto mexicano, y Google Dataset Search para búsquedas académicas.
...
Los foros de Canvas se estructuran con post inicial obligatorio, respuestas a compañeros, y síntesis integradora opcional. Esta estructura operacionaliza el principio conectivista de que ``el aprendizaje ocurre a través de redes'' \citep{Siemens2005}.

\subsection{Gamificación como Estrategia Didáctica}

Como estrategia complementaria a nivel micro, se propone implementar una tabla de clasificación voluntaria por equipos y badges digitales por logros específicos. Desde la perspectiva de multiliteracidades \citep{Cope2009}, estos elementos funcionan como ``textos'' multimodales que comunican logros y motivan mediante retroalimentación visual inmediata, transformando el aprendizaje en una experiencia donde el estudiante participa activamente en un sistema de significados interconectados.

\subsection{Integración de Analytics y Redes Sociales}

El currículum actual trata la tecnología como herramienta de presentación más que como fuente de datos. Se proponen tres actividades concretas: (1) análisis de tendencias con Google Trends antes del taller de design thinking, (2) simulación de A/B testing conceptual donde los estudiantes predicen engagement de mensajes alternativos, y (3) reflexión crítica sobre algoritmos siguiendo a \citet{Apple1979}: ``¿Cómo determinan qué consumidores `vemos' y cuáles quedan invisibles?''
...
Los cambios propuestos integran tres marcos teóricos complementarios que no operan de forma aislada: cada cambio curricular propuesto integra simultáneamente al menos dos de ellos.
...
\midrule
Conectivismo \citep{Siemens2005} & Aprendizaje en redes y comunidades de práctica & Kaggle como comunidad, foros Canvas, redes sociales, IA como nodo \\
\bottomrule
\end{tabular}
\end{table}
...
Esta propuesta opera en el nivel micro porque los objetivos de aprendizaje están definidos a nivel meso y requieren aprobación institucional. Sin embargo, si los cambios demuestran efectividad, podrían escalarse formalmente incorporando objetivos de reflexión crítica y competencia digital con datos abiertos. Este enfoque de ``innovación desde el aula'' es consistente con la visión de \citet{Stenhouse1975} del docente como investigador de su propia práctica.
